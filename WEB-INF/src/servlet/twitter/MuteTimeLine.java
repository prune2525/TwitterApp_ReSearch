...
@WebServlet(urlPatterns = { "/mTimeLine" })
public class MuteTimeLine extends HttpServlet{
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException{
		...
		if(null != session.getAttribute("accessToken")){
			...
			try {
				...
				MuteSelect mdata = new MuteSelect();
				List<String> word = mdata.select(str);
				word.add(str);
				...
				for(Status status : statuses){
					...
					suser = suser.replaceAll(suser, "<a href=https://twitter.com/"
					+ suser + ">" + "@" + suser + "</a>");
					//クライアント
					...
					for(URLEntity link : url){
						...
						twtext = twtext.replaceAll(tmp, "<a href=" + tmp + ">" + tmp + "</a>");
					}
					//テキスト内のメディア
					for(MediaEntity media : murl){
						String tmedia = media.getURL();
						twtext = twtext.replaceAll(tmedia, "<a href=" + tmedia + ">" + tmedia + "</a>");
					}
					//テキスト内のハッシュタグ
					...
					boolean flag = true;
					for(String mute : word){
						if(twtext.indexOf(mute) != -1){
							System.out.println("ヒット単語：" + mute + " 本文：" + twtext);
							flag = false;
							break;
						}
					}
